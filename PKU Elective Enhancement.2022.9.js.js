...
window.getClass = function () {
    ...
    while (classLine) {
        ...
        let info = {
            ...
            id: i.firstElementChild.href ? i.firstElementChild.href.match(/BZ[0-9_]+$/) : []
        };
        ...
        while (flag && i) {
            ...
            //如果是人数，则拆分开来，但还有考虑2倍bug
                ...
                zzz.incidence.bind(i, "click", function (e) {
                    ...
                    var id = "";
                    var name = "???";
                    var element = zzz.query("a", goToClass(e.target), function (x) {
                        if (id == "" && x.href) {
                            id = x.href.match(/BZ[0-9_]+$/)&&x.href.match(/BZ[0-9_]+$/)[0] || 0;
                            name = x.innerText;
                        }
                    });
                    console.log(id);
...
window.autoSelect = function () {
    zzz.incidence.bind(code, "change", function () {
        if (!EnableAutoSelect) return false;
