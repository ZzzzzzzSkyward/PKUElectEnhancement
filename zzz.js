...
zzz.api.render={
    ...
    simpleTag:function (text) {
        if(!text) return "This is a simple tag explanation:"+
            "Input:[string]."+
            "Tag:must be included in [] at the beginning of any paragraph. \\n marks the end of one paragraph."+
            "Usage:multiple tags are divided with blank character"+
            this.explanation.join("\n");
        var paragraph=text.split("\n");
        var result=[];
        for(let i of paragraph){
            let tags=i.match(/^\[[^[]+]/);
            let className="",addition="";
            if(!tags) result.push("<p>"+i+"</p>");
            else{
                tags=tags[0].slice(1,tags[0].length-1).split(" ");
                for(let j of tags){
                    j=j.trim();
                    if(!j.length) continue;
                    if(j==="b") className+="bold ";
                    else if(j==="i") className+="italic ";
                    else if(j==="u") className+="underline";
                    else if(j.slice(0,2)==="fs"&&j[2]==="=") addition+="font-size:"+j.slice(3)+"em;";
                    else if(j[0]==="c"&&j[1]==="=") addition+="color:"+j.slice(2)+";";
                    else if(j[0]==="o"&&j[1]==="=") addition+="opacity:"+j.slice(2)+";";
                    else className+=j+" ";
                }
                result.push("<p class='"+className+"' style='"+addition+"'>"+i.replace(/^\[[^[]+]/,"")+"</p>");
            }
        }
        return result.join("\n");
    },
    explanation:["title:class title","b:bold","i:italic","u:underline","class=className","c:color","o:opacity","fs:relative font size","a:align"]
};
